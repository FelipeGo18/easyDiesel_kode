...
const ICONS_DIR = path.join(processCwd, 'frontend', 'public', 'icons');

const baseSvg = (id, label, paths) => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 110 130" fill="none">
<defs>
  <style>
    .hex    { fill: #0a0700; stroke: #F5A623; stroke-width: 1.4; stroke-linejoin: miter; }
    ...
    .f      { fill: #F5A623; }
    .f-bg   { fill: #2a1a00; }
    .lbl    { font-family: monospace; font-size: 9px; fill: #383838; text-anchor: middle; }
  </style>
</defs>
<g id="icon-${id}">
  <path class="hex" d="M55 5 L98 30 L98 80 L55 105 L12 80 L12 30 Z"/>
  ${paths}
  <text class="lbl" x="55" y="122">${label.toUpperCase()}</text>
</g>
</svg>`;
...
icons.forEach(icon => {
  const content = baseSvg(icon.id, icon.label, icon.paths);
  fs.writeFileSync(path.join(ICONS_DIR, \`\${icon.id}.svg\`), content);
});
